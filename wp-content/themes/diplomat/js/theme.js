		...
		var self = {
			...
			masonryReload: function () {
				...
				var old_post_key = 0,
					...
					title_symbols = $grid.data('titlesymbols'),
					excerpt_symbols = $grid.data('excerptsymbols'),
					show_excerpt = $grid.data('showexcerpt'),
					image_background = $grid.data('imagebackground'),
					image_opacity = $grid.data('imageopacity'),
					masonrySpiner = $('.masonry-loader');
						...
						$.post(ajaxurl, data, function(response) {
							...
							$grid.imagesLoaded(function(){
								...
								if (response_lm_posts) {
									$grid.next('.post-load-more').empty();
									$grid.next('.post-load-more').append(response_lm);
								} else {
									$grid.next().next('.post-load-more').remove();
								}
