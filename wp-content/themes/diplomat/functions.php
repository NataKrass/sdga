...
add_action('wp_enqueue_scripts', 'tmm_wp_enqueue_scripts');
...
if ( !function_exists('tmm_admin_enqueue_scripts') ) {
	function tmm_admin_enqueue_scripts() {
        ...
        wp_register_script($handle, TMM_THEME_URI . '/admin/js/general.js', array('jquery'), true, true);
        ...
        $translation_array = array(
            ...
            'loading' => esc_html__("Loading", 'diplomat'),
            'sure' => esc_html__("Are you sure?", 'diplomat'),
            'title_slide_posts' => esc_html__("Posts", 'diplomat'),
            'tmm_theme_options_url' =>  admin_url('themes.php?page=tmm_theme_options&tmm_action=save_options'),
            'tmm_enter_new_color_scheme_name' =>  esc_html__("Please enter new color scheme name!", 'diplomat'),
            'tmm_enter_new_color' =>  esc_html__("Please enter color of new scheme!", 'diplomat'),
            'tmm_creating_new_color_scheme' =>  esc_html__("Creating new color scheme, please wait...", 'diplomat'),
            'tmm_new_color_scheme_added' =>  esc_html__("New color scheme added!", 'diplomat'),
            'is_IE' => (int) $is_IE
        );

        wp_localize_script($handle, 'tmm_l10n', $translation_array);
		...
		wp_enqueue_script('tmm_colorpicker', TMM_THEME_URI . '/admin/js/colorpicker/colorpicker.js', array('jquery'), true, true);
		wp_enqueue_media();
		...
		if (isset($_GET['page']) && $_GET['page'] == 'tmm_theme_options') {
			...
			wp_enqueue_script('tmm_theme_options', TMM_THEME_URI . '/admin/theme_options/js/options.js', array('jquery'), true, true);
			wp_enqueue_script('tmm_theme_cache', TMM_THEME_URI . '/admin/js/js.cookie.js', array('jquery'), true, true);
			wp_enqueue_script('tmm_theme_custom_sidebars', TMM_THEME_URI . '/admin/theme_options/js/custom_sidebars.js', array('jquery'), true, true);
			wp_enqueue_script('tmm_theme_seo_groups', TMM_THEME_URI . '/admin/theme_options/js/seo_groups.js', array('jquery'), true, true);
			wp_enqueue_script('tmm_theme_form_constructor', TMM_THEME_URI . '/admin/theme_options/js/form_constructor.js', array('jquery'), true, true);
			wp_enqueue_script('tmm_theme_selectivizr', TMM_THEME_URI . '/admin/theme_options/js/selectivizr-and-extra-selectors.min.js', array('jquery'), true, true);
		}
...
add_action('admin_enqueue_scripts', 'tmm_admin_enqueue_scripts');
		...
		return array(
			...
			*/
			...
			array(
				...
				'source'   => 'https://github.com/ThemeMakers/tmm_content_composer/archive/diplomat_v1.1.4.zip',
				'required' => true,
				'version'  => '1.1.4',
			),
			array(
				...
				'source'   => 'https://github.com/ThemeMakers/tmm_events_calendar/archive/diplomat_v1.1.1.zip',
				'required' => false,
				'version'  => '1.1.1',
			),
			array(
				...
				'source'   => 'https://downloads.wordpress.org/plugin/seamless-donations.5.0.21.zip',
				'required' => false,
			),
			array(
			...
			array(
				...
				'slug'     => 'bbpress',
				'source'   => 'https://downloads.wordpress.org/plugin/bbpress.2.6.4.zip',
				'required' => false,
			),
			array(
				'name'     => esc_html__( 'LayerSlider', 'diplomat' ),
				'slug'     => 'LayerSlider',
				'source'   => TMM_THEME_URI . '/install/layersliderwp-6.11.2.zip',
				'required' => false,
			),
			array(
